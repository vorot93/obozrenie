...
- [maxminddb](https://github.com/maxmind/MaxMind-DB-Reader-python) - for geolocation support. Obozrenie reads a MaxMind DB country database (`.mmdb`) and shows server locations as country flag emoji. It probes common system locations (e.g. `/usr/share/GeoIP/`); if none is found, the GTK frontend offers to download a free [DB-IP](https://db-ip.com/) country database into your user cache. No MaxMind account is required.
